...
## DataBase
### Primary Key가 무엇인가요?
후보 키 중 선택한 주 키로써, 각 행을 구분하는 유일한 열을 의미합니다. 그렇기 때문에 NULL값과 중복된 값을 가질 수 없습니다.

<br />

#### Super Key는 무엇인가요?
각 row를 유일하게 식별할 수 있는 하나 또는 그 이상의 속성들의 집합입니다. 유일성만 만족하면 슈퍼기라 할 수 있습니다.
...
#### Primary Key와 Foreign Key에 대해 설명해주세요
...
2. Consistency로 일관성입니다. 트랜잭션이 성공적으로 완료 되면, 일관적인 DB 상태를 유지함을 의미합니다. `자료형 int가 string으로 변하거나 하면 안 된다는 뜻입니다.`
3. Isolatioin으로 독립성입니다. 둘 이상의 트랜잭션이 동시에 실행되는 경우, 서로의 연산에 영향을 끼치지 않음을 말하며, `수행 중인 트랜잭션은 완전히 완료가 될 때까지 다른 트랜잭션에서 그 결과를 참조할 수 없습니다.`
...
#### 비정규화는 무엇인가요?
정규화된 테이블을 **읽기 성능 향상**을 위해 다시 합치는 것을 의미합니다. 데이터 무결성을 떨어뜨리는 것을 감안합니다.
...
#### Spring에서 DI에 대해서 설명해보세요.
**DI(Dependency Injection, 의존성 주입, 의존 관계)** 와 **DI 컨테이너**를 제공하여 **다형성과 OCP, DIP**를 가능하도록 지원합니다. 상세하게 설명하자면, **클라이언트 코드의 변경 없이 기능을 확장**할 수 있으며, 마치 부품을 교체하듯이 개발할 수 있음을 의미합니다.

클린아키텍처 관점에서 말씀드리면, 클린아키텍처는 관심사를 Layer 별로 구분하고, 고수준부터 저수준으로 관리하는 아키텍처인데, 저수준에 있는 Layer를 변경함으로써 고수준에 있는 Layer에 영향을 끼치지 않도록 합니다. 
