...
#### 완전 이진 트리와 전 이진 트리, 그리고 포와 이진 트리에 대해서 설명해주세요.
...
### Sync(동기)와 Async(비동기)의 차이가 무엇인가요?
메서드를 실행시킴과 동시에 반환 값이 기대되는 경우(값이 반환될 때까지 blocking)를 동기라고 표현하고, 그렇지 않은 경우(blocking 없이 이벤트 큐에 넣거나 백그라운드 스레드에게 해당 task 위임 후 다음 코드 실행)를 비동기라고 합니다. 

**block non-block 추가 예정**
...
#### 비정규화(반정규화)는 무엇인가요?
조인으로 인한 성능 저하가 예상될 때, **읽기 성능 향상**을 위해 정규화된 테이블을 다시 합칩는 것으로, 데이터 무결성을 떨어뜨리는 것을 감안합니다.
