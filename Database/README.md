...
|행 및 열 형식의 테이블 기반이며 표준 **스키마 정의를 엄격하게 준수**합니다. 다중 행 트랜잭션이 필요한 응용 프로그램에 유리합니다.|문서, 키-값 쌍, 그래프 또는 열을 기반으로 구성할 수 있으며 **표준 스키마 정의를 요구하지 않기 때문에, 미리 정의된 구조 없이 유연하게 저장할 수 있습니다.**|
|확장 비용이 많이 듭니다.|확장 비용이 상대적으로 저렴합니다.|
|단일 서버에서 하드웨어 용량(CPU, RAM, SSD 등)을 늘려 **수직 확장(Scale Up)할 수 있습니다.**|대규모로드를 관리하고 힙을 줄이기 위해 인프라에 더 많은 서버를 추가하여 **수평 확장(Scale Out)이 가능합니다.**|
|쿼리 처리를 위한  **표준 인터페이스가 있으므로** 복잡한 쿼리에 적합합니다.|**표준 인터페이스가 없기 때문에** 복잡한 쿼리에는 적합하지 않습니다.|
|**ACID**를 올바르게 따릅니다.|**Brewer's CAP 정리**(일관성, 가용성 및 파티션 허용 오차)를 올바르게 따릅니다.|
|새 데이터를 추가하려면 스키마 변경과 같은 일부 변경이 필요합니다.|그냥 삽입합니다.|
|우수한 공급 업체 지원 및 커뮤니티 지원이 제공됩니다.|제한된 커뮤니티만 사용할 수 있습니다.|
|MySQL, Oracle, MS-SQL, SQLite|MongoDB, Apache CouchDB, Redis Hbase|
